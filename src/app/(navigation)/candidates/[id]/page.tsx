...
export default function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex gap-x-[40px] mt-[134px] w-[1225px] bg-[#FFFCF7] m-auto shadow rounded-[17.54px] py-[59px] px-[61px]">
      <div className="flex flex-col w-[314px]">
        <div className='h-[354px] relative' >
          ...
          <div className='relative b-0 bg-white h-[86.26px] w-[212.85px] z-1 m-auto flex flex-col place-content-center rounded-[10px]'>
            ...
            <div className='flex justify-center'>
              <p className='text-[14px] text-center text-[#5F5F75]'>{candidate.party}</p>
            </div>
          </div>
        </div>
        <Card className="grow mt-[80px]">
          <h1>Quotes</h1>
          <p className="text-[14px] text-[#5F5F75]">{candidate.quotes}</p>
        </Card>

        <Card className="grow mt-[58px]">
          <h1>What other said about {candidate.name}</h1>
          <p className="text-[14px] text-[#5F5F75]">{candidate.said}</p>
        </Card>
      </div>
      <div className="grow flex flex-col gap-y-[50px]">
        <Card className="grow">
          <h1>Bio</h1>
          <p className="text-[14px] text-[#5F5F75]">{candidate.bio}</p>
        </Card>
        <Card className="grow">
          <h1>Education</h1>
          <p className="text-[14px] text-[#5F5F75]">{candidate.education}</p>
        </Card>
        <Card className="grow">
          <h1>Goals</h1>
          <p className="text-[14px] text-[#5F5F75]">{candidate.goals}</p>
        </Card>
      </div>
    </div>
  )
